...
@onready var prefix_drop: OptionButton = $PanelContainer/MarginContainer/ScrollContainer/VBoxContainer/Prefix/OptionButton
...
var version: Dictionary = {"major": 0, "minor": 0, "patch": 0, "prefix": ""}
...
func _connect_signals() -> void:
	...
	prefix_drop.item_selected.connect(_on_version_changed.bind("prefix"))
...
func _update_version_from_project() -> void:
	...
	var raw_version = project_settings.version
	
	# Split by dash to isolate the prefix string safely
	var prefix_split = raw_version.split("-")
	var base_version = prefix_split[0]
	var version_parts = base_version.split(".")
	
	if version_parts.size() < 3:
		push_error("Failed to parse project version format correctly.")
		return
		
	version.major = version_parts[0].to_int()
	version.minor = version_parts[1].to_int()
	version.patch = version_parts[2].to_int()
	version.prefix = prefix_split[1] if prefix_split.size() > 1 else ""
...
func _get_version_string() -> String:
	if version.prefix != "":
		return "%d.%d.%d-%s" % [version.major, version.minor, version.patch, version.prefix]
	return "%d.%d.%d" % [version.major, version.minor, version.patch]


func _get_last_commit_dict() -> Dictionary:
	if last_commit == "None":
		return {"major": 0, "minor": 0, "patch": 0, "prefix": ""}
		
	var prefix_split = last_commit.split("-")
	var base_version = prefix_split[0]
	var array = base_version.split(".")
	
	if array.size() < 3:
		return version
		
	var pref = prefix_split[1] if prefix_split.size() > 1 else ""
	return {"major": array[0].to_int(), "minor": array[1].to_int(), "patch": array[2].to_int(), "prefix": pref}
...
func _is_newer() -> bool:
	...
	if version.patch > last_version.patch:
		return true
	elif version.patch < last_version.patch:
		return false
		
	# Fallback check if prefix changes but numbers are identical
	if version.prefix != last_version.prefix:
		return true
...
func _update_spin_boxes() -> void:
	...
	# OptionButtons must find the matching item text index manually
	prefix_drop.selected = -1
	for i in prefix_drop.item_count:
		if prefix_drop.get_item_text(i) == version.prefix:
			prefix_drop.selected = i
			break
...
func _reset() -> void:
	...
	version = {"major": 0, "minor": 0, "patch": 0, "prefix": ""}
	last_commit = "None"
	commit_label.set_text("Last Commit: %s" % last_commit)
	_update_spin_boxes()
	commit_message.clear()
...
func _on_version_changed(value: float, type: String) -> void:
	match type:
		...
		"prefix":
			var idx = int(value)
			if idx >= 0:
				version.prefix = prefix_drop.get_item_text(idx)
			else:
				version.prefix = ""
			_update_spin_boxes()
